...
import com.wedapp.weddingplanner.organizer.checklist.wedding.todolist.allLeading.adapter.VendorAdapterLeading;
...
import com.wedapp.weddingplanner.organizer.checklist.wedding.todolist.allLeading.models.vendor.VendorListModel;
...
import com.wedapp.weddingplanner.organizer.checklist.wedding.todolist.databinding.ActivityVendorListBinding;
...
public class VendorListActivityLeading extends BaseActivityRecyclerBindingLeading implements EasyPermissions.PermissionCallbacks, EasyPermissions.RationaleCallbacks {
...
//                return;
    ...
    public void fillFromDB() {
        ...
        try {
            arrayList = this.db.vendorDao().getAll(AppPrefLeading.getCurrentEvenId(this.context));
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (arrayList.size() > 0) {
    ...
    public void setRecycler() {
        ...
        this.binding.recycler.setAdapter(new VendorAdapterLeading(this.context, this.model.getArrayList(), new RecyclerItemClick() {
    ...
    public void openItemDetail(int i, int i2, VendorRowModel vendorRowModel, boolean z) {
        Intent intent = new Intent(this.context, AddEditVendorActivityLeading.class);
        intent.putExtra(AddEditVendorActivityLeading.EXTRA_IS_EDIT, z);
        intent.putExtra(AddEditVendorActivityLeading.EXTRA_POSITION, i);
        intent.putExtra(AddEditVendorActivityLeading.EXTRA_POSITION_MAIN, i2);
        intent.putExtra(AddEditVendorActivityLeading.EXTRA_MODEL, vendorRowModel);
    ...
    private int getSelectedPosById() {
        for (int i = 0; i < this.orderTypeList.size(); i++) {
            if (this.orderTypeList.get(i).getLabel().equalsIgnoreCase(AppPrefLeading.getSortTypeVendor(this.context))) {
                return i;
            }
        }
        return 0;
    ...
    public void setOrderTypeListDialog() {
        ...
        this.dialogOrderTypeListBinding.recycler.setAdapter(new SelectionAdapterLeading(this.context, true, this.orderTypeList, new RecyclerItemClick() {
            public void onClick(int i, int i2) {
                ...
                AppPrefLeading.setSortTypeVendor(context, ((SelectionRowModel) orderTypeList.get(selectedOrderTypePos)).getLabel());
                notifyAdapter();
                try {
    ...
    private void sortBy() {
        char c;
        String sortTypeVendor = AppPrefLeading.getSortTypeVendor(this.context);
        ...
        if (intent != null) {
            try {
                if (intent.hasExtra(AddEditTaskActivityLeading.EXTRA_MODEL)) {
                    this.rowModel = (VendorRowModel) intent.getParcelableExtra(AddEditTaskActivityLeading.EXTRA_MODEL);
