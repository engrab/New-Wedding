...
import com.example.weddingplanner.allLeading.adapter.GuestAdapterLeading;
...
import com.example.weddingplanner.allLeading.models.guest.GuestListModel;
...
public class GuestListActivityLeading extends BaseActivityRecyclerBindingLeading implements EasyPermissions.PermissionCallbacks, EasyPermissions.RationaleCallbacks {
...
//                return;
    ...
    public void fillFromDB() {
        ...
        try {
            arrayList = this.db.guestDao().getAllGuest(AppPrefLeading.getCurrentEvenId(this.context));
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (arrayList.size() > 0) {
            for (int i = 0; i < arrayList.size(); i++) {
                ...
                try {
                    ...
                    guestRowModel.getArrayList().addAll(this.db.guestDao().getAllComp(AppPrefLeading.getCurrentEvenId(this.context), guestRowModel.getId()));
    ...
    public void setRecycler() {
        ...
        this.binding.recycler.setAdapter(new GuestAdapterLeading(this.context, this.model.getArrayList(), new RecyclerItemClick() {
    ...
    public void openItemDetail(int i, int i2, GuestRowModel guestRowModel, boolean z) {
        Intent intent = new Intent(this.context, AddEditGuestActivityLeading.class);
        intent.putExtra(AddEditGuestActivityLeading.EXTRA_IS_EDIT, z);
        intent.putExtra(AddEditGuestActivityLeading.EXTRA_POSITION, i);
        intent.putExtra(AddEditGuestActivityLeading.EXTRA_POSITION_MAIN, i2);
        intent.putExtra(AddEditGuestActivityLeading.EXTRA_MODEL, guestRowModel);
    ...
    private int getSelectedPosById() {
        for (int i = 0; i < this.orderTypeList.size(); i++) {
            if (this.orderTypeList.get(i).getLabel().equalsIgnoreCase(AppPrefLeading.getSortTypeGuest(this.context))) {
                return i;
            }
        }
        return 0;
    ...
    public void setOrderTypeListDialog() {
        ...
        this.dialogOrderTypeListBinding.recycler.setAdapter(new SelectionAdapterLeading(this.context, true, this.orderTypeList, new RecyclerItemClick() {
            public void onClick(int i, int i2) {
                ...
                AppPrefLeading.setSortTypeGuest(context, ((SelectionRowModel) orderTypeList.get(selectedOrderTypePos)).getLabel());
                notifyAdapter();
                try {
    ...
    private void sortBy() {
        char c;
        String sortTypeGuest = AppPrefLeading.getSortTypeGuest(this.context);
        ...
        if (intent != null) {
            try {
                if (intent.hasExtra(AddEditTaskActivityLeading.EXTRA_MODEL)) {
                    this.rowModel = (GuestRowModel) intent.getParcelableExtra(AddEditTaskActivityLeading.EXTRA_MODEL);
